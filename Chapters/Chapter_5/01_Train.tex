...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Loss term 3}
    \end{subfigure}
    \caption{TensorBoard monitoring for the DEIMv2 training on the Site~1 dough-ball dataset. The plots report the evolution of validation accuracy and loss terms over the 200-epoch training schedule. As can be observed, the best validation checkpoint was reached before the end of the training schedule, confirming that the selected model corresponds to a favorable validation regime.}
    \label{fig:DEIMv2_training_curves1}
\end{figure}

The final detector achieved an Average Precision (AP) of 0.929 and an Average Recall (AR) of 0.953 on the evaluation split, indicating that the model was able to localize dough balls with high reliability despite the challenging visual configuration of the site. %This dataset is more complex than the one collected at Site~2 because dough balls may touch or partially overlap along the production flow, making instance separation less straightforward.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/DEIMv2_Results1.png}
...
The second training experiment was carried out on the bread-loaf dataset collected at Site~2, which consisted of approximately 70,000 images partitioned into training, validation, and test subsets with an 80\% - 10\% - 10\% split. Also in this case, Figure~\ref{fig:DEIMv2_training_curves2} shows a stable optimization process and supports the checkpoint selected through the validation-loss criterion.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Loss term 3}
    \end{subfigure}
    \caption{TensorBoard monitoring for the DEIMv2 training on the Site~2 bread-loaf dataset. The plots report the evolution of validation accuracy and loss terms over the 200-epoch training schedule. As can be observed, the best validation checkpoint was reached before the end of the training schedule, confirming that the selected model corresponds to a favorable validation regime.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/DEIMv2_Results2.png}
...
\subsubsection{Result comparison and discussion}
