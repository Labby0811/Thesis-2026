...
The first training experiment was carried out on the dough-ball dataset collected at Site~1, which consisted of approximately 50k images partitioned into training, validation, and test subsets with an 80/10/10 split. Figures~\ref{fig:DEIMv2_training_curves1a} and~\ref{fig:DEIMv2_training_curves1b} summarize the main TensorBoard plots recorded during training. Overall, the curves indicate a stable optimization process, with progressive convergence of the loss terms and a validation trend that supports the selected final checkpoint.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Loss term 1}
    \end{subfigure}
    \caption{First part of the TensorBoard monitoring for the DEIMv2 training on the Site~1 dough-ball dataset. The plots report the evolution of validation accuracy and of one main loss term over the 200-epoch training schedule.}
    \label{fig:DEIMv2_training_curves1a}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Loss term 3}
    \end{subfigure}
    \caption{Second part of the TensorBoard monitoring for the DEIMv2 training on the Site~1 dough-ball dataset. The plots report the evolution of the remaining loss terms over the 200-epoch training schedule.}
    \label{fig:DEIMv2_training_curves1b}
\end{figure}

The final detector achieved an Average Precision (AP) of 0.929 and an Average Recall (AR) of 0.953 on the evaluation split, indicating that the model was able to localize dough balls with high reliability despite the challenging visual configuration of the site. The Site~1 dataset is more difficult than the one collected at Site~2, since dough balls may touch or partially overlap along the production flow, making instance separation and bounding-box assignment less straightforward.
...
The second training experiment was carried out on the bread-loaf dataset collected at Site~2, which consisted of approximately 70k images partitioned into training, validation, and test subsets with an 80/10/10 split. Also in this case, the TensorBoard plots in Figures~\ref{fig:DEIMv2_training_curves2a} and~\ref{fig:DEIMv2_training_curves2b} show a stable optimization process and support the checkpoint selected through the validation-loss criterion.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Loss term 1}
    \end{subfigure}
    \caption{First part of the TensorBoard monitoring for the DEIMv2 training on the Site~2 bread-loaf dataset. The plots report the evolution of validation accuracy and of one main loss term over the 200-epoch training schedule.}
    \label{fig:DEIMv2_training_curves2a}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Loss term 3}
    \end{subfigure}
    \caption{Second part of the TensorBoard monitoring for the DEIMv2 training on the Site~2 bread-loaf dataset. The plots report the evolution of the remaining loss terms over the 200-epoch training schedule.}
    \label{fig:DEIMv2_training_curves2b}
\end{figure}
